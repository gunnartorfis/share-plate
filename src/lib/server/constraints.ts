...
export const getMyConstraints = createServerFn({ method: "GET" }).handler(async () => {
...
export const getDayTemplates = createServerFn({ method: "GET" }).handler(async () => {
  const user = await getUser()
  if (!user) throw new Error("Unauthorized")
  return db.select().from(dayTemplates).where(eq(dayTemplates.userId, user.id))
})
...
export const saveDayTemplate = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const existing = await db
      ...
      .from(dayTemplates)
      .where(
        and(eq(dayTemplates.userId, user.id), eq(dayTemplates.dayOfWeek, data.dayOfWeek)),
      )
      .limit(1)
